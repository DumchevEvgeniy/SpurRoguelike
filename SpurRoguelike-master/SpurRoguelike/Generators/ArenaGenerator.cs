...
namespace SpurRoguelike.Generators {
    internal class ArenaGenerator : LevelGenerator {
        public ArenaGenerator(Int32 seed, NameGenerator nameGenerator)
        ...
        public override Level Generate(LevelGenerationSettings settings, IList<MonsterClass> monsterClasses,
            IList<ItemClass> itemClasses, Int32 levelNumber) {
            var level = base.Generate(settings, monsterClasses, itemClasses, levelNumber);

            level.Spawn(new Location(), new OpenExitTrigger("OpenExit", Int32.MaxValue));
