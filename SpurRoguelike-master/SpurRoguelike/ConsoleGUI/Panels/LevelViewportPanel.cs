...
namespace SpurRoguelike.ConsoleGUI.Panels {
    internal class LevelViewportPanel : Panel {
        ...
        public override void RedrawContents() {
            ...
            for(Int32 x = Math.Max(0, -leftOffset); x < Math.Min(level.Field.Width, ClientZone.Width - leftOffset); x++) {
                for(Int32 y = Math.Max(0, -topOffset); y < Math.Min(level.Field.Height, ClientZone.Height - topOffset); y++) {
        ...
        public Location TranslateCoords(Int32 clientLeft, Int32 clientTop) {
            return new Location(clientLeft - LevelLeftOffset, clientTop - LevelTopOffset);
        }

        public void GetPlayerScreenCoords(out Int32 left, out Int32 top) {
        ...
        private Int32 LevelLeftOffset => ClientZone.Left + ClientZone.Width / 2 - levelProvider()?.Player?.Location.X ?? 0;

        private Int32 LevelTopOffset => ClientZone.Top + ClientZone.Height / 2 - levelProvider()?.Player?.Location.Y ?? 0;
